...
Look at the number of counties for each state:

```{r}
state_counts <- data.frame(table(grpd_demog_county$abb))
colnames(state_counts) <- c("abb", "n_counties")
county_counts <- read.csv(paste0(local_data,"county_numbers.csv"))
state_names <- county_counts$info[seq(2,306, 6)]
n_counties <- county_counts$info[seq(4,308, 6)]
county_counts <- data.frame(abb = state_names,
                            n_counties_correct = n_counties)

comparison <- left_join(state_counts, county_counts, by = "abb")
comparison["diff"] <- as.numeric(comparison$n_counties) - as.numeric(comparison$n_counties_correct)
#comparison[comparison$diff!=0,]
va <- acs_data[str_starts(acs_data$fixed_fip, "51"),]
#table(va$in_df,useNA = "ifany")
...
m3.slopes["x"] <- c(.15, .325,.34,.15)
m3.slopes["y"] <- c(.15,.1,.115,.17)
