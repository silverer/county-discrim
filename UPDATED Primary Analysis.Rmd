...
modlist <- list(m0.ut,m2,m3)
reg_out <- format_regression_table(modlist)
m2a.sum <- format_regression_table(list(m0.ut,m2a))
m2a.sum <- m2a.sum %>% 
...
Note: The change in R2 needs to be manually updated in the results spreadsheet so that the 3-way interaction model is compared to the 2-way interaction model with conservatism as a moderator (rather than unemployment).
...
m2a.sum <- format_regression_table(list(m1,m2a), reg_fmt=reg_fmt_tmp)
m2a.sum <- m2a.sum %>% 
  rename(model2a_b=model2_b,
         model2a_se=model2_se_b) %>% 
  select(-starts_with("model1"))
anova(m2,m3)
reg_out <- left_join(reg_out, m2a.sum, by = "Predictor")
...
Format outputs (copy and paste these into a spreadsheet)

```{r}
tmp_fmt <- read.csv("data/regression_format_rbst_2.csv")
modlist <- list(m1,m2,m3)
reg_out <- format_regression_table(modlist, reg_fmt=tmp_fmt)
m2a.sum <- format_regression_table(list(m1,m2a), reg_fmt=tmp_fmt)
m2a.sum <- m2a.sum %>% 
  rename(model2a_b=model2_b,
         model2a_se=model2_se_b) %>% 
  select(-starts_with("model1"))
...
reg_out <- left_join(reg_out, m2a.sum, by = "Predictor")
write.xlsx(reg_out, paste0(output,
                           "robustness_temporal.xlsx"),overwrite = T)
